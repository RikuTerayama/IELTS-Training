/**
 * Idiomシードスクリプト
 * data/idiom/master.ts から表現登録＋問題生成
 */

import { createClient } from '@supabase/supabase-js';
import { IDIOM_MASTER } from '../data/idiom/master';
...
function getCategoryUsage(category: string): string {
  if (category.includes('common')) return '日常会話';
  if (category.includes('opinion')) return '意見表現';
  if (category.includes('time')) return '時間表現';
  if (category.includes('essay')) return 'エッセイ';
  return '熟語';
}

async function seedIdiom() {
  try {
    console.log('Starting Idiom seed...');
    console.log(`Total items in master: ${IDIOM_MASTER.length}`);
    ...
    for (const item of IDIOM_MASTER) {
      const normalized = normalizeExpression(item.expression);
    ...
    console.log(`Total idiom questions in database: ${questionStats?.length || 0}`);
