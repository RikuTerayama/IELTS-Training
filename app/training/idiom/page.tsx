...
function IdiomPageContent() {
  ...
  const handleSkillSelect = async (skill: 'speaking' | 'writing' | 'listening' | 'reading') => {
    ...
    try {
      const response = await fetchLexiconSets(skill, 'idiom');
      if (response.ok && response.data) {
        const data = response.data;
        const setsData = data.sets;
        if (Object.keys(setsData).length === 0) {
  ...
  const getCategoryLabel = (category: string): string => {
    ...
    if (parts[0] === 'writing') {
      if (parts[1] === 'essay') {
        return `Writing / Essay / ${parts[2]?.charAt(0).toUpperCase() + parts[2]?.slice(1) || ''}`;
      }
      return `Writing / ${parts.slice(1).map(p => p.charAt(0).toUpperCase() + p.slice(1)).join(' / ')}`;
    } else if (parts[0] === 'speaking') {
      if (parts[1] === 'common') {
        return `Speaking / Common Idioms`;
      } else if (parts[1] === 'opinion') {
        return `Speaking / Opinion Idioms`;
      } else if (parts[1] === 'time') {
        return `Speaking / Time Idioms`;
      }
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 min-w-0">
              ...
              <button onClick={() => handleSkillSelect('listening')} disabled={loading}
                ...
                <div className={cn('text-sm', cardDesc)}>句動詞・会話表現・口語パラフレーズ</div>
              </button>
              <button onClick={() => handleSkillSelect('speaking')} disabled={loading}
                ...
                <div className={cn('text-sm', cardDesc)}>スピーキングで使う熟語</div>
              </button>
              <button onClick={() => handleSkillSelect('writing')} disabled={loading}
                ...
                <div className={cn('text-sm', cardDesc)}>ライティングで使う熟語</div>
              </button>
            </div>
          </div>
        )}
            ...
            {selectedCategory && selectedMode && (
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  ...
                  >
                    {loading ? '読み込み中...' : selectedSkill === 'listening' && (sets[selectedCategory].due_click > 0 || sets[selectedCategory].due_typing > 0) ? '復習＋新規で開始' : '開始'}
                  </button>
                </div>
              </div>
            )}
            ...
            {quizState.currentIndex >= quizState.questions.length ? (
              ...
              <>
                ...
                {(() => {
                  ...
                  /* 結果表示（Listening は explanation と meta を表示） */
                  <div className={cn('p-6', cardBase)}>
